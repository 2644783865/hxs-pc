...
namespace MaSoft.UI
{
    public partial class Setting : Form
    {
        ...
        private void Setting_Load(object sender, EventArgs e)
        {
            ...
            {
                var user = db.Queryable<User>().FirstOrDefault();
        ...
        private void checkBox_login_CheckedChanged(object sender, EventArgs e)
        {
            using (var db = SugarDao.GetInstance())
            {
                ...
                if (user != null)
                {
                    db.Update<User>(new { IsAutoLogin = checkBox_login.Checked }, it => it.Id == user.Id); 
                }
                else
                {
                    checkBox_login.Checked = false;
        ...
        private void checkBox_top_CheckedChanged(object sender, EventArgs e)
        {
            using (var db = SugarDao.GetInstance())
            {
                ...
                if (user != null)
                {
                    db.Update<User>(new { IsTop = checkBox_top.Checked }, it => it.Id == user.Id); 
                    var launch = new Launch {TopMost = checkBox_top.Checked};
                }
                else
                {
                    checkBox_top.Checked = false;
        ...
        private void checkBox_power_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                using (var db = SugarDao.GetInstance())
                {
                    ...
                    if (user != null)
                    {
                        db.Update<User>(new { IsStartRun = checkBox_power.Checked }, it => it.Id == user.Id); 
                        SetMeAutoStart(checkBox_power.Checked);
                    }
                    else
                    {
                        checkBox_power.Checked = false;
